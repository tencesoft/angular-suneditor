...
import { InjectionToken, EventEmitter, Component, Optional, Inject, Input, Output, Pipe, NgModule } from '@angular/core';
...
'use strict';
var index = { blockquote, align, font, fontSize, fontColor, hiliteColor, horizontalRule, list, table, formatBlock, lineHeight, template, paragraphStyle, textStyle, link, image, video, audio, math, imageGallery };

/**
 * Injection token for options object
 */
const SUNEDITOR_OPTIONS = new InjectionToken('SUNEDITOR_OPTIONS');
/**
 * Feature Key used internal for editors injected as singleton
 */
const ROOT_INJECTOR_KEY = 'root_injector_key';

class NgxSuneditorComponent {
    constructor(ngZone, options) {
        this.ngZone = ngZone;
        this.created_event = new EventEmitter();
        this.onload_event = new EventEmitter();
        this.onScroll_event = new EventEmitter();
        this.onMouseDown_event = new EventEmitter();
        this.onClick_event = new EventEmitter();
        this.onInput_event = new EventEmitter();
        this.onKeyDown_event = new EventEmitter();
        this.onKeyUp_event = new EventEmitter();
        this.onChange_event = new EventEmitter();
        this.onFocus_event = new EventEmitter();
        this.onBlur_event = new EventEmitter();
        this.showController_event = new EventEmitter();
        this.toggleFullScreen_event = new EventEmitter();
        this.toggleCodeView_event = new EventEmitter();
        this.showInline_event = new EventEmitter();
        this.audioUploadHandler_event = new EventEmitter();
        this.onAudioUpload_event = new EventEmitter();
        this.videoUploadHandler_event = new EventEmitter();
        this.onVideoUpload_event = new EventEmitter();
        this.imageUploadHandler_event = new EventEmitter();
        this.onImageUpload_event = new EventEmitter();
        this.onCut_event = new EventEmitter();
        this.onCopy_event = new EventEmitter();
    ...
    ngAfterViewInit() {
        this.ngZone.runOutsideAngular(() => {
            ...
            this.editor = suneditor.create(document.getElementById(`ngxsuneditor_${this.editorID}`) ||
                `ngxsuneditor_${this.editorID}`, this.options);
        });
        this.defineEvents();
    ...
    setToolbarButtons_fn(buttonList) {
        this.editor.setToolbarButtons(buttonList);
    }
    setOptions_fn(options) {
        this.editor.setOptions(options);
    }
    setDefaultStyle_fn(style) {
        this.editor.setDefaultStyle(style);
    }
    noticeOpen_fn(message) {
    ...
    getFilesInfo_fn(pluginName) {
        return this.editor.getFilesInfo(pluginName);
    }
    insertImage_fn(files) {
        this.editor.insertImage(files);
    }
    insertHTML_fn(html, notCleaningData, checkCharCount, rangeSelection) {
        this.editor.insertHTML(html, notCleaningData, checkCharCount, rangeSelection);
    }
    setContents_fn(contents) {
        this.editor.setContents(contents);
    }
    appendContents_fn(contents) {
    ...
    commandHandler_fn(element, command) {
...
NgxSunViewComponent.ɵcmp = i0.ɵɵngDeclareComponent({ minVersion: "12.0.0", version: "12.0.5", type: NgxSunViewComponent, selector: "ngx-sunview", inputs: { content: "content", bypassSantiziser: "bypassSantiziser" }, ngImport: i0, template: `
    ...
    <div
      ...
      *ngIf="!bypassSantiziser"
      [innerHTML]="content"
    ></div>
  `, isInline: true, styles: ["@import \"../../../../node_modules/suneditor/src/assets/css/suneditor-contents.css\";"], directives: [{ type: i1$1.NgIf, selector: "[ngIf]", inputs: ["ngIf", "ngIfThen", "ngIfElse"] }], pipes: { "safeHtml": SafeHtmlPipe } });
...
export { NgxSunViewComponent, NgxSuneditorComponent, NgxSuneditorModule, ROOT_INJECTOR_KEY, SUNEDITOR_OPTIONS, SafeHtmlPipe, align, audio, blockquote, font, fontColor, fontSize, formatBlock, hiliteColor, horizontalRule, image, imageGallery, lineHeight, link, list, math, paragraphStyle, table, template, textStyle, video };
//# sourceMappingURL=ngx-suneditor.js.map
